...
28.
```sql
CREATE OR REPLACE TRIGGER TG_COUNTRYCODE_IS_ONLY_UPPERCASE_LETTERS
BEFORE INSERT OR UPDATE ON COUNTRY 
FOR EACH ROW 
BEGIN
  
  -- Enlève les chiffre du CODE
  SELECT REGEXP_REPLACE(:NEW.CODE, '[0-9]', '') INTO :NEW.CODE FROM DUAL;
  
  -- Passe en majuscule
  SELECT UPPER(:NEW.CODE) INTO :NEW.CODE FROM DUAL;

END;
/

UPDATE COUNTRY SET CODE='f3' WHERE CODE='F';
```
